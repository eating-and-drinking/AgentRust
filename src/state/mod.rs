...
pub struct AppState {
    ...
    /// MERIT metacognition engine. Optional so single-shot CLI commands
    /// (`agentrust config get …` etc.) don't pay the construction cost.
    pub metacog: Arc<RwLock<MetacognitionEngine>>,
    /// Running flag
...
impl AppState {
    ...
    pub fn new(settings: Settings) -> Self {
        let metacog_cfg = settings.metacog.to_engine_config();
        Self {
            settings,
            ...
            metacog: Arc::new(RwLock::new(MetacognitionEngine::with_config(metacog_cfg))),
