...
impl TeamMemorySyncService {
    ...
    pub async fn is_authenticated(&self) -> bool {
        ...
        let token_path = home.join(".agentrust").join(".team_token");
        token_path.exists()
    }
    ...
    async fn load_local_memories(&self) -> anyhow::Result<()> {
        let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
        let team_dir = home.join(".agentrust").join("team");
    ...
    pub async fn delete_memory(&self, id: &str) -> anyhow::Result<()> {
        let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
        let memory_path = home.join(".agentrust").join("team").join(format!("{}.json", id));
